...
def create_password(length, include_letters=True, include_numbers=True, include_symbols=True):
    ...
    # Add letters to the character pool if selected
    if include_letters:
        char_pool += string.ascii_letters
    
    # Add digits to the character pool if selected
    if include_numbers:
        char_pool += string.digits
    
    # Add punctuation to the character pool if selected
    if include_symbols:
...
generated_pwd = create_password(pwd_length, use_letters, use_numbers, use_symbols)
